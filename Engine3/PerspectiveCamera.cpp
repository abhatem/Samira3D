

#include "PerspectiveCamera.h"

#include <glm/gtc/quaternion.hpp>
#include <cmath>



PerspectiveCamera::PerspectiveCamera()
{
	//currentRotatioinAroundX = 0;
	world_up = glm::vec3(0, 1, 0);
	m_last_mouseX = 0;
	m_last_mouseY = 0;
	m_forward = glm::vec3(0, 0, 1);
	m_pitch = 0;
	m_yaw = 0;
	m_roll = 0;
}

//PerspectiveCamera::PerspectiveCamera(RenderTarget *renderTexture)
//{
//	m_renderTexture = renderTexture;
//}

void PerspectiveCamera::rotate(float pitch, float yaw)
{
	m_pitch += pitch;
	m_yaw += yaw;

	normalizeAngles();
}

void PerspectiveCamera::rotate(float pitch, float yaw, float roll)
{
	m_pitch += pitch;
	m_yaw += yaw;
	m_roll += roll;
	normalizeAngles();
}



void PerspectiveCamera::setProjection(float fieldOfView, float aspect, float zNear, float zFar)
{
	m_fieldOfView = fieldOfView;
	m_aspect = aspect;
	m_zNear = zNear;
	m_zFar = zFar;
}


void PerspectiveCamera::move(const glm::vec3 &localTranslation)
{
	m_transform.move(localTranslation);
}

float PerspectiveCamera::getPitch() const
{
	return m_pitch;
}

void PerspectiveCamera::setPitch(float pitch)
{
	m_pitch = pitch;
	normalizeAngles();
}

float PerspectiveCamera::getYaw() const
{
	return m_yaw;
}

void PerspectiveCamera::setYaw(float yaw)
{
	m_yaw = yaw;
	normalizeAngles();
}

float PerspectiveCamera::getRoll() const
{
	return m_roll;
}
void PerspectiveCamera::setRoll(float roll)
{
	m_roll = roll;
	normalizeAngles();
}

glm::vec3 PerspectiveCamera::getForward()
{
	glm::vec4 forward = glm::inverse(orientation()) * glm::vec4(0, 0, -1, 1);
	return glm::vec3(forward);
}

glm::vec3 PerspectiveCamera::getUp()
{
	glm::vec4 up = glm::inverse(orientation()) * glm::vec4(0, 1, 0, 1);
	return glm::vec3(up);
}

glm::vec3 PerspectiveCamera::getRight()
{
	glm::vec4 right = glm::inverse(orientation()) * glm::vec4(1, 0, 0, 1);
	return glm::vec3(right);
}


void PerspectiveCamera::lookAt(Vec3 target)
{
	// assert(target != m_transform.getTranslation());

	// glm::vec3 direction = glm::normalize(target-m_transform.getTranslation());
	// float dot = glm::dot(glm::vec3(0, 0, 1), direction);
	// if (fabs(dot - (-1.0f)) < 0.000001f) {
	//     m_transform.setRotation(glm::angleAxis(glm::degrees(M_PI), glm::vec3(0, 1, 0)));
	//     return;
	// }
	// else if (fabs(dot - (1.0f)) < 0.000001f) {
	//     m_transform.setRotation( glm::quat());
	//     return;
	// }

	// float angle = -glm::degrees(acosf(dot));

	// glm::vec3 cross = glm::normalize(glm::cross(glm::vec3(0, 0, 1), direction));
	// m_transform.setRotation( glm::normalize(glm::angleAxis(angle,
	// cross)));

	// m_transform.setRotation(glm::lookAt(m_transform.getTranslation(), target, VECTOR_UP));
	// normalizeAngles();

}


glm::mat4 PerspectiveCamera::view()
{
	// return orientation() * glm::translate(glm::mat4(), -m_transform.getTranslation());

	return orientation() * glm::inverse(m_transform.getTranslationMatrix());


	// return glm::inverse(orientation()); // * glm::inverse(m_transform.getTranslationMatrix());


}

glm::mat4 PerspectiveCamera::orientation()
{
	// glm::mat4 orientation = glm::mat4(1);
	// orientation = glm::rotate(orientation, glm::radians(m_pitch), glm::vec3(1, 0, 0));
	// orientation = glm::rotate(orientation, glm::radians(m_yaw), glm::vec3(0, 1, 0));
	// return orientation;

	m_transform.setRotation(glm::radians(m_pitch), glm::vec3(1, 0, 0));
	m_transform.rotate(glm::radians(m_roll), glm::vec3(0, 0, 1));
	m_transform.rotate(glm::radians(m_yaw), glm::vec3(0, 1, 0));
	
	return glm::toMat4(m_transform.getRotation());
}

glm::mat4 PerspectiveCamera::projection()
{
	return glm::perspective(m_fieldOfView, m_aspect, m_zNear, m_zFar);
}

glm::mat4 PerspectiveCamera::matrix()
{
	return projection() * view();
}



void PerspectiveCamera::normalizeAngles()
{
	m_yaw = fmodf(m_yaw, 360.0f);
	//fmodf can return negative values, but this will make them all positive
	if (m_yaw < 0.0f)
		m_yaw += 360.0f;


}
