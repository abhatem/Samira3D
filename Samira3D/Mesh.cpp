...
void Mesh::addVertices(std::vector<Vertex> &vertices, std::vector<unsigned int> &indices)
{
	...
	for (int i = 0; i < vertices.size(); i++)
	{
		...
		m_colors.push_back(vertices[i].getColor().x);
		m_colors.push_back(vertices[i].getColor().y);
		m_colors.push_back(vertices[i].getColor().z);
	}
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * m_colors.size(), m_colors.data(), GL_STATIC_DRAW);
